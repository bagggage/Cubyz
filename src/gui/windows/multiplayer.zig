...
const SelectableList = @import("../components/SelectableList.zig");
...
var serverList: *SelectableList = undefined;
...
fn initServerElement(server: *const ServerInfo) *HorizontalList {
...
fn selectServer(serverIdx: usize, _: usize) void {
...
pub fn onOpen() void {
	...
	serverList = SelectableList.init(.{0, 0}, list.maxHeight/2, 0, .{.callback = &selectServer});
	for(servers.items) |*server| {
		serverList.add(initServerElement(server));
	}
